...
export default function RCAPage() {
  ...
  // Reset hasPopulated when navigating to a different incident's RCA page,
  // so the new incident's data populates the form correctly.
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.rootCause.trim()) {
      setFormError("Root cause is required.");
      return;
    }
    if (form.rootCause.trim().length < 10) {
      setFormError("Root cause must be at least 10 characters.");
      return;
    }
    if (!form.resolution.trim()) {
      setFormError("Resolution is required.");
      return;
    }
    if (form.resolution.trim().length < 10) {
    ...
    try {
      const payload = {
        ...form,
        contributingFactors: factors.filter((f) => f.trim()),
      };
      if (isEdit) {
        await api.patch(`/rca/${id}`, payload);
      } else {
        await api.post(`/rca`, { ...payload, incidentId: id });
      }
      setSuccessMsg(
          ...
          <div className="card">
            ...
            <form onSubmit={handleSubmit} className="space-y-4" noValidate>
              ...
              {successMsg && (
                <div
                  role="status"
                  className="bg-green-50 border border-green-100 text-green-700 text-sm px-4 py-3 rounded-xl"
                >
                  {successMsg}
                </div>
              )}
              ...
              <div>
                ...
                {factors.length === 0 && (
                  <p className="text-xs text-gray-400">No factors added yet.</p>
                )}
              ...
              <div className="flex justify-end gap-3 pt-2">
                <Link to={`/incidents/${id}`} className="btn-secondary">
                  Cancel
                </Link>
                <button
                  type="submit"
